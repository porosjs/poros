...
import { Navigate, type IRouteProps } from 'poros';
  ...
  if (microAppName) {
    if (route.children?.length) {
      ...
      if (childrenRouteHasComponent) {
        throw new Error(
          `[@porosjs/plugin-qiankun]: You can not attach micro app ${microAppName} to route ${route.path} whose children has own component!`,
        );
      }
    }

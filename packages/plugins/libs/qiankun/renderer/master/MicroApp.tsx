  ...
  (componentProps: Props, componentRef: Ref<MicroAppType>) => {
    ...
    useEffect(() => {
      if (!appConfig) {
        setComponentError(
          new Error(
            `[@porosjs/plugin-qiankun]: Can not find the configuration of ${name} app!`,
          ),
        );
      }
      return noop;
            ...
            if (canUpdate(microApp)) {
              ...
              if (process.env.NODE_ENV === 'development') {
                if (Date.now() - updatingTimestamp.current < 200) {
                  console.warn(
                    `[@porosjs/plugin-qiankun] It seems like microApp ${name} is updating too many times in a short time(200ms), you may need to do some optimization to avoid the unnecessary re-rendering.`,
                  );
                }

                console.info(
                  `[@porosjs/plugin-qiankun] MicroApp ${name} is updating with props: `,
                  props,
                );

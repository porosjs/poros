...
export default (api: IApi) => {
  ...
  api.addRuntimePlugin(() => {
    return [withTmpPath({ api, path: 'renderer/slaveRuntimePlugin.ts' })];
  });
  ...
  api.onGenerateFiles({
    fn() {
      ...
      api.writeTmpFile({
        path: 'index.ts',
        content: `
export { connectMaster } from './renderer/connectMaster';
export { MicroAppLink } from './renderer/MicroAppLink';
      `,
      });
    },
    before: 'model',

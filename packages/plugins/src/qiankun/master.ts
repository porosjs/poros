...
export default (api: IApi) => {
  ...
  api.addRuntimePlugin(() => {
    return [withTmpPath({ api, path: 'renderer/masterRuntimePlugin.tsx' })];
  });
...
export interface IRuntimeConfig {
  ...
  ${MODEL_EXPORT_NAME}?: () => Record<string, any>;
    ...
    api.writeTmpFile({
      path: 'renderer/masterOptions.ts',
      content: `
...
options = { ...options, apps: [ ...options.apps, ...localStore.get('apps') ]};
...
export const setMasterOptions = (newOpts) => options = ({ ...options, ...newOpts });
    ...
    api.writeTmpFile({
      path: 'renderer/MicroAppLoader.tsx',
    ...
    api.writeTmpFile({
      path: 'index.ts',
      content: `
export { MicroApp } from './renderer/MicroApp';
export { MicroAppWithMemoHistory } from './renderer/MicroAppWithMemoHistory';
      `,
    });
    ...
    api.writeTmpFile({
      ...
      content: Mustache.render(mainMasterOptionsTpl, {
        QiankunApps: JSON.stringify(api.config.master?.apps ?? []),
      }),
    });
  });
};
